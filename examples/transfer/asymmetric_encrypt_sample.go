...
func NewUserAsymmetricEncryptSample() {
...
// 密钥迁移前示例代码。
func BeforeMigrateAsymmetricEncryptSample() {
	...
	request := &kms20160120.AsymmetricEncryptRequest{
	...
	result, err := client.AsymmetricEncrypt(request)
	if err != nil {
		panic(err)
	}
...
// 密钥迁移后示例代码。
func AfterMigrateAsymmetricEncryptSample() {
	// 创建kms共享网关config并设置相应参数
	config := &openapi.Config{
		// 设置地域Id
		RegionId: tea.String("your-region-id"),
...
func AsymmetricEncrypt(client *sdk.TransferClient) {
	request := &kms20160120.AsymmetricEncryptRequest{
		Plaintext:    tea.String("your-plaintext"),
		KeyId:        tea.String("your-key-id"),
		KeyVersionId: tea.String("your-key-version-id"),
	...
	result, err := client.AsymmetricEncryptWithOptions(request, runtime)
	if err != nil {
		panic(err)
	}
...
func main() {
	NewUserAsymmetricEncryptSample()
	BeforeMigrateAsymmetricEncryptSample()
	AfterMigrateAsymmetricEncryptSample()
}
