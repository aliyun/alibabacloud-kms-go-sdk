...
// 密钥迁移前示例代码。
func BeforeMigrateDecryptSample() {
...
// 密钥迁移后示例代码。
func AfterMigrateDecryptSample() {
	// 创建kms共享网关config并设置相应参数
	config := &openapi.Config{
		// 设置地域Id
		RegionId: tea.String("your-region-id"),
...
func Decrypt(client *sdk.TransferClient) {
	request := &kms20160120.DecryptRequest{
		CiphertextBlob: tea.String("your-ciphertext-blob"),
	}
	...
	result, err := client.DecryptWithOptions(request, runtime)
	if err != nil {
		panic(err)
	}
...
func main() {
	NewUserDecryptSample()
	BeforeMigrateDecryptSample()
	AfterMigrateDecryptSample()
}
