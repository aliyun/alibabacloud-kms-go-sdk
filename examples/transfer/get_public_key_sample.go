...
func NewUserGetPublicKeySample() {
...
// 密钥迁移前示例代码。
func BeforeMigrateGetPublicKeySample() {
...
// 密钥迁移后示例代码。
func AfterMigrateGetPublicKeySample() {
	// 创建kms共享网关config并设置相应参数
	config := &openapi.Config{
		// 设置地域Id
		RegionId: tea.String("your-region-id"),
...
func GetPublicKey(client *sdk.TransferClient) {
	request := &kms20160120.GetPublicKeyRequest{
		KeyId:        tea.String("your-key-id"),
		KeyVersionId: tea.String("your-key-version-id"),
	}
	...
	result, err := client.GetPublicKeyWithOptions(request, runtime)
	if err != nil {
		panic(err)
	}
...
func main() {
	NewUserGetPublicKeySample()
	BeforeMigrateGetPublicKeySample()
	AfterMigrateGetPublicKeySample()
}
