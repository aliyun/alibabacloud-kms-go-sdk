...
func NewUserAsymmetricVerifySample() {
...
// 密钥迁移前示例代码。
func BeforeMigrateAsymmetricVerifySample() {
	// 创建kms共享网关config并设置相应参数
	config := &openapi.Config{
		// 设置地域Id
		RegionId: tea.String("your-region-id"),
	...
	request := &kms20160120.AsymmetricVerifyRequest{
		Value:        tea.String("your-value"),
		Digest:       tea.String("your-digest"),
...
// 密钥迁移后示例代码。
func AfterMigrateAsymmetricVerifySample() {
	// 创建kms实例网关config并设置相应参数
	kmsConfig := &dkmsopenapi.Config{
		// 设置请求协议为https
		Protocol: tea.String("https"),
...
func AsymmetricVerify(client *sdk.TransferClient) {
	request := &kms20160120.AsymmetricVerifyRequest{
		Value:        tea.String("your-value"),
		Digest:       tea.String("your-digest"),
		KeyId:        tea.String("your-key-id"),
	...
	result, err := client.AsymmetricVerifyWithOptions(request, runtime)
	if err != nil {
		panic(err)
	}
...
func main() {
	NewUserAsymmetricVerifySample()
	BeforeMigrateAsymmetricVerifySample()
	AfterMigrateAsymmetricVerifySample()
}
